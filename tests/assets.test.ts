...
describe('temporary asset manifest', () => {
  it('keeps the complete fixed-facing inventory relative and visibly pending', () => {
    const manifest = JSON.parse(readFileSync(new URL('../public/assets/manifest.json', import.meta.url), 'utf8')) as { status: string; items: Array<{ id: string; file: string; frames: number; dimensions: number[]; fps: number; facing: string; pivot: number[]; trimmed: boolean; source: string }> };
